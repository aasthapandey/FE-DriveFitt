import { StaticPageData } from "@/types/staticPages";

export const runningData: StaticPageData = {
  title: "Running at DriveFitt",
  description: "Join India's first professional indoor running studio.",
  seoTitle: "Indoor Running Studio | DriveFitt",
  seoDescription:
    "Experience the best indoor running track and training at DriveFitt.",
  hero: {
    title: "Running Redefined. Only at DRIVE FITT.",
    description:
      "India's First Professional Indoor Running Studio — Where Running Becomes a Lifestyle",
    desktopImage: "/images/hero/running.svg",
    mobileImage: "/images/hero/running-mobile.svg",
    btnPrimaryText: "Book a free trial",
    btnSecondaryText: "Join Online",
  },
  cardSection4: {
    title: "Run Stronger, Run Smarter",
    description: "Join India's first professional indoor running studio.",
    cardSection: [
      {
        title: "Race day prep",
        description:
          "Exclusive group race training programs so you feel confident on race days.",
        backgroundImage: "/images/card-section/running/race-prep.svg",
        link: "",
      },
      {
        title: "Precision Indoor Track",
        description:
          "Climate-controlled 200m track with shock-absorbing surface for joint-friendly, year-round training.",
        backgroundImage: "/images/card-section/running/indoor-track.svg",
        link: "",
      },
      {
        title: "HIIT + Intervals + Recovery",
        description: "Massage that hits the right spot.",
        backgroundImage: "/images/card-section/running/hiit.svg",
        link: "",
      },
      {
        title: "Electric studio vibe",
        description:
          "Pulsing lights, energizing playlists, and community camaraderie.",
        backgroundImage: "/images/card-section/running/studio-vibe.svg",
        link: "",
      },
      {
        title: "Coach-powered motivation",
        description:
          "Expert runners who inspire, instruct, and hold you accountable.",
        backgroundImage: "/images/card-section/running/coach.svg",
        link: "",
      },
    ],
  },
};
